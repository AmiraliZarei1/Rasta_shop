        ...
        if register.is_valid():
            ...
            if password == password:
                ...
                if user is not None:
                    ...
                    new_user = User(username=username , email=email, password=password, is_active=False , verify_code=random_code.random_number())
                    new_user.set_password(password)
                    new_user.save()
                    # new_user.generate_token()
                    send_mail('تایید ایمیل' , user.email , {'user' : user} , 'email_template.html')
        ...
        if user is not None and not user.is_active:
            ...
            if confirm_form.is_valid():
                ...
                if user.verify_code == confirm:
                    ...
                    confirm_form.add_error('code' , 'کد اشتباه است')
            else:
                raise Http404("test")
